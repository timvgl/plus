...
Ferromagnet::Ferromagnet(std::shared_ptr<System> system_ptr, 
    ...
    : Magnet(system_ptr, name),
      ...
      msat(name + ":msat", "A/m", system(), 1.0),
      aex(name + ":aex", "J/m", system(), 0.0),
      ku1(name + ":ku1", "J/m3", system(), 0.0),
      ku2(name + "ku2", "J/m3", system(), 0.0),
      kc1(name + ":kc1", "J/m3", system(), 0.0),
      kc2(name + ":kc2", "J/m3", system(), 0.0),
      kc3(name + "kc3", "J/m3", system(), 0.0),
      alpha(name + ":alpha", "", system(), 0.0),
      temperature(name + ":temperature", "K", system(), 0.0),
      xi(name + ":xi", "", system(), 0.0),
      Lambda(name + ":lambda", "", system(), 0.0),
      freeLayerThickness(name + ":free_layer_thickness", "m",
                         system(), grid().size().z * cellsize().z),
      fixedLayerOnTop(true),
      epsilonPrime(name + ":epsilon_prime", "", system(), 0.0),
      fixedLayer(name + ":fixed_layer", "m", system(), {0, 0, 0}),
      pol(name + ":pol", "", system(), 0.0),
      anisU(name + ":anisU", "", system(), {0, 0, 0}),
      anisC1(name + ":anisC1", "", system(), {0, 0, 0}),
      anisC2(name + ":anisC2", "", system(), {0, 0, 0}),
      jcur(name + ":jcur", "A/m2", system(), {0, 0, 0}),
      biasMagneticField(name + ":bias_magnetic_field", "T", system(), {0, 0, 0}),
      dmiTensor(system()),
      enableDemag(true),
      enableOpenBC(false),
      enableZhangLiTorque(true),
      enableSlonczewskiTorque(true),
      appliedPotential(name + ":applied_potential", "V", system(), std::nanf("0")),
      conductivity(name + ":conductivity", "S/m", system(), 0.0),
      amrRatio(name + ":amr_ratio", "", system(), 0.0),
